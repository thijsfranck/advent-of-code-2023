...
def count_total_cards(path: Path) -> int:
    """Count the total number of cards."""
    won_cards = Counter[str]()

    def count(card: Card) -> int:
        """Count the card."""
        card_count = 1 + won_cards.get(card.card_id, 0)
        card_number = int(card.card_id)
        matches = len(card.matching_numbers)

        won_cards.update({str(card_number + i + 1): card_count for i in range(matches)})

        return card_count

    return sum(count(card) for card in Parser(read_char_stream(path)))


if __name__ == "__main__":
    logging.basicConfig(level=logging.DEBUG)

    solution = count_total_cards(Path(__file__).parent / "input/input")
